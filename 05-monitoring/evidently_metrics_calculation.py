...
def prep_db():
    with psycopg.connect("host=localhost port=5432 user=postgres password=postgres", autocommit=True) as conn:
        ...
        with psycopg.connect("host=localhost port=5432 dbname=test user=postgres password=postgres") as conn:
            conn.execute(create_table_statement)


@task
def calculate_metrics_postgresql(curr, i):
    current_data = raw_data[(raw_data.lpep_pickup_datetime >= (begin + timedelta(i))) &
    ...
    curr.execute(
        ...
        (begin + timedelta(i), prediction_drift,
